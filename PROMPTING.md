...
> **Spectacle / VFX as first-class concern**: Pass 1 outputs a `spectacleHints[]` array — 2–6 planned VFX/action moments distributed across segments. Pass 2 renders each one inside the relevant segment's timestamp blocks using a Seedance-validated VFX vocabulary (time-freeze ripples, plasma arcs, telekinetic lifts, slow-mo impact rings, particle blooms, holographic shatters, controlled fireballs, frost advances, etc.). Each effect must be anchored to a body part or object with specified origin and trajectory — "her index finger → 2-metre cyan plasma arc → the lock". Disembodied "magic happens" descriptions are forbidden. One VFX moment per timestamp block max — stacking collapses Seedance.
